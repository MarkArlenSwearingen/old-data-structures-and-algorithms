...
const howMuchPencil = (str) => {
  ...
  for(let i = 0; i < str.length-1; i++){
    let str1 = str.slice(i, str.length);
    result.push(str1);
  }
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  gruffaloCrumble.ingredients.forEach((element) => {
    ...
    let newIngredients = newWords.splice(2, element.length);
    let newIngreditentsString = newIngredients.join(' ');
    result.push(newIngreditentsString);
  });
  return result;
};
